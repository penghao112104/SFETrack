...
def compute_grids(features, strides):
    ...
    for level, feature in enumerate(features):
        ...
        grids_per_level = torch.stack((shift_x, shift_y), dim=1) + strides[level] / 2.0
...
def get_center3x3(locations, centers, strides, range=3):
    ...
    return (dist_x <= strides_expanded[:, :, 0] * range_val) & \
           (dist_y <= strides_expanded[:, :, 0] * range_val)
