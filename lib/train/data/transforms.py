...
class Normalize(TransformBase):
    ...
    def transform_image(self, image):
        ...
        norm_channels = len(self.mean)
        if channel_count == norm_channels:
            return tvisf.normalize(image, self.mean, self.std, self.inplace)

        if channel_count % norm_channels == 0:
            chunks = torch.split(image, norm_channels, dim=0)
            chunks = [tvisf.normalize(x, self.mean, self.std, self.inplace) for x in chunks]
            return torch.cat(chunks, dim=0)

        raise ValueError(
            "Normalize expected {} channels or a multiple of {}, got {}.".format(
                norm_channels, norm_channels, channel_count
            )
        )
        ...
        if do_grayscale:
            ...
            if image.shape[2] % 3 == 0:
                ...
                for i in range(0, image.shape[2], 3):
                    img_gray = cv.cvtColor(image[:, :, i:i + 3], cv.COLOR_RGB2GRAY)
                    gray_images.append(np.stack([img_gray, img_gray, img_gray], axis=2))
                return np.concatenate(gray_images, axis=2)
            raise ValueError("ToGrayscale expected 3 channels or a multiple of 3, got {}.".format(image.shape[2]))
