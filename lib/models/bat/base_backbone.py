...
class BaseBackbone(nn.Module):
    ...
    def finetune_track(self, cfg, patch_start_index=1):
        ...
        if new_patch_size != self.patch_size:
            print('Patch size differs from the pretrained model; interpolating weights.')
        ...
        P_H = self.img_size[0] // self.patch_size
        P_W = self.img_size[1] // self.patch_size

        patch_pos_embed = patch_pos_embed.view(B, E, P_H, P_W)

        H, W = search_size
        new_P_H = H // new_patch_size
        new_P_W = W // new_patch_size
        ...
        if self.add_sep_seg:
            self.template_segment_pos_embed = nn.Parameter(
                torch.zeros(1, 1, self.embed_dim)
            )

            self.template_segment_pos_embed = trunc_normal_(
                self.template_segment_pos_embed,
                std=.02
            )

            self.search_segment_pos_embed = nn.Parameter(
                torch.zeros(1, 1, self.embed_dim)
            )

            self.search_segment_pos_embed = trunc_normal_(
                self.search_segment_pos_embed,
                std=.02
            )
