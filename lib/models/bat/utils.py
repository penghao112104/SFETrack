...
def resolve_encoder_layer_indices(indices, depth):
    out = set()
    for idx in indices:
        i = int(idx)
        if i == -1:
...
def resolve_moe_layer_indices(moe_cfg, depth):
    insert_layer = getattr(moe_cfg, "INSERT_LAYER", -1)
    layer = int(insert_layer)
    if layer == -1:
        return [depth - 1]
    if layer < 1 or layer > depth:
        raise ValueError(
            f"MODEL.MOE.INSERT_LAYER must be in [1, {depth}] or -1, got {insert_layer}."
        )
    return [layer - 1]
...
def combine_tokens(template_tokens, search_tokens, mode='direct', return_res=False):
    ...
    if mode == 'direct':
        ...
        template_tokens = template_tokens.view(
            B,
            Hp // window_size,
            window_size,
            W,
        ...
        template_tokens = torch.cat(
            [template_tokens[:, 0, ...], template_tokens[:, 1, ...]],
            dim=2
        )
...
def recover_tokens(merged_tokens, len_template_token, len_search_token, mode='direct'):
    ...
    if mode == 'direct':
        ...
        recovered_tokens = torch.cat(
            (template_tokens, first_half, second_half),
            dim=1
        )
